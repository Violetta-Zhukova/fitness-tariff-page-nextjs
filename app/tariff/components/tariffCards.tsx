      ...
      {tariffs.map((tarif) => {
        ...
        return (
          <div
            ...
            key={tarif.id}
            className={`relative flex flex-row 3xl:flex-col h-full justify-start items-center gap-7.5 xs:gap-12.5 3xl:gap-10 rounded-[20px] 3xl:rounded-[40px] p-5 xs:py-5 xs:px-7.5 3xl:pb-5.5 3xl:pt-17.5 3xl:px-4.5 cursor-pointer hover:border-[#81FE95] transition-all duration-300 ease-in-out ${isSelected ? "bg-[#81FE95]/15 border-2 border-[#81FE95] scale-102 shadow-lg shadow-[#81FE95]/20" : "bg-[#313637] border-2 border-[#484D4E]"}`}
          >
            <div
              className={`absolute flex items-center justify-center px-1.5 py-0.75 3xl:px-2 3xl:py-1.25 content-[''] -top-0.5 left-54.25 xs:left-65.5 3xl:left-12.75 w-10.5 h-5.75 xs:w-12 xs:h-6.75 3xl:w-16.5 3xl:h-9.75 rounded-b-lg font-gilroy text-[13px] xs:text-base xs:leading-[20.8px] 3xl:text-[22px] 3xl:leading-[28.6px] leading-4 font-medium text-white bg-[#FD5656] z-10 transition-all duration-500 ease-in-out ${isDiscount ? "opacity-100 scale-100" : "opacity-0 scale-95 pointer-events-none"}`}
            >
              -{discount}%
            </div>
            ...
            <div className="flex flex-col items-start 3xl:items-center gap-3 xs:gap-4 3xl:gap-7.5">
              ...
              <div className="flex flex-col items-end">
                <p className="font-semibold text-3xl xs:text-[34px] 3xl:text-[50px]">
